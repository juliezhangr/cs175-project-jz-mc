...
//   Julie Zhang and Matthiew Chartier
//
////////////////////////////////////////////////////////////////////////
...
static shared_ptr<SgRbtNode> g_skyNode, g_groundNode, g_boxNode, g_ballNode;
...
static void drawArcBall(const ShaderState& curSS) {
  ...
  Matrix4 MVM = rigTFormToMatrix(inv(getPathAccumRbt(g_world, g_currentCameraNode)) * getArcballRbt())
                * Matrix4::makeScale(Cvec3(g_arcballRadius/0.5));
...
static Cvec3 getArcballDirection(const Cvec2& p, const double r) {
  ...
  if (n2 >= r*r)
    ...
    return normalize(Cvec3(p, std::sqrt(r*r - n2)));
}
...
static RigTForm moveArcball(const Cvec2& p0, const Cvec2& p1) {
  ...
  if (!getScreenSpaceCircle(Cvec3(eyeInverse * Cvec4(arcballCenter, 1)),
                            g_arcballRadius, projMatrix, g_frustNear, g_frustFovY, g_windowWidth, g_windowHeight,
                            ballScreenCenter, ballScreenRadius))
    return RigTForm();

  const Cvec3 v0 = getArcballDirection(p0 - ballScreenCenter, ballScreenRadius);
  const Cvec3 v1 = getArcballDirection(p1 - ballScreenCenter, ballScreenRadius);
...
static void keyboard(const unsigned char key, const int x, const int y) {
  ...
  case 'h':
    ...
    << "<space>\t\tCopy state to current frame\n"
    << "u\t\tCopy current frame to state\n"
    << "n\t\tCreate new frame and move to it\n"
    << "d\t\tDelete frame\n"
    << ">\t\tGo to next frame\n"
    << "<\t\tGo to prev. frame\n"
  ...
  case 'v':
  {
    shared_ptr<SgRbtNode> viewers[] = {g_skyNode, g_ball};
    for (int i = 0; i < 3; ++i) {
  ...
  case 'p':
    ...
    std::cerr << "Picking mode is " << (g_pickingMode ? "on" : "off") << endl;
    break;
  case 'm':
    g_activeCameraFrame = SkyMode((g_activeCameraFrame+1) % 2);
    std::cerr << "Editing sky eye w.r.t. " << (g_activeCameraFrame == WORLD_SKY ? "world-sky frame\n" : "sky-sky frame\n") << endl;
    break;
  case 'a':
...
static void constructRobot(shared_ptr<SgTransformNode> base, const Cvec3& color) {
  ...
  ShapeDesc shapeDesc[NUM_SHAPES] = {
    ...
    {2, -ARM_LEN/2, 0, 0, ARM_LEN, ARM_THICK, ARM_THICK, g_sphere}, // upper left arm
    ...
    {5, 0, -LEG_LEN/2, 0, LEG_THICK, LEG_LEN, LEG_THICK, g_sphere}, // upper right leg
    {6, 0, -LEG_LEN/2, 0, LEG_THICK, LEG_LEN, LEG_THICK, g_sphere}, // upper left leg
    ...
    {9, 0, HEAD_SIZE/2 * 1.5, 0, HEAD_SIZE, HEAD_SIZE, HEAD_SIZE, g_sphere}, // head
  };
