...
public:
  ...
  virtual bool visit(SgShapeNode& shapeNode) {
    ...
    /*  C1.restLength = sqrt(abs(dot(B.absPos - A.absPos, B.absPos - A.absPos))); */

    /*  C2.particleA = C.particle_id; */
    /*  C2.particleB = B.particle_id; */
    /*  C2.restLength = sqrt(abs(dot(C.absPos - B.absPos, C.absPos - B.absPos))); */

    /*  C3.particleA = A.particle_id; */
    /*  C3.particleB = C.particle_id; */
    /*  C3.restLength = sqrt(abs(dot(A.absPos - C.absPos, A.absPos - C.absPos))); */
    ...
    /* GeoPtr->setVertices(addedV_); */
    return true;
  }

  virtual bool postVisit(SgShapeNode& shapeNode) {
    /* for (int i = 0; i < addedV_; i++) */
    /*   particleStack_.pop_back(); */
    return true;
  }
};
